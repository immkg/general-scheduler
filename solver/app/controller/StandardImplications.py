...
class StandardImplications():
    ...
    def init_vars(self):
        ...
        for k in range(
                ...
                1):  # should it not be from 1 to pmax rather than from 0 to pmax - 1
    ...
    def basic_implications(self):
        ...
        for (t, s, g, n, d, p1) in list(self.graph['x!tsgndp'].keys()):
        ...
        for (t, s, g, n, d, p2) in list(self.graph['xtsgndp'].keys()):
            or_list = []
            for p1 in StaticVariables.periods[d]:
                if bic2(t, s, g, n, d, p1, p2):
                    or_list.append(('x!tsgndp', (t, s, g, n, d, p1)))
            self.graph['xtsgndp'][(t, s, g, n, d, p2)].append(
                (StaticVariables.or_head, or_list))

        for (t, s, g, n, d, p) in list(self.graph['xtsgndp'].keys()):
            self.graph['xtsgndp'][(t, s, g, n, d, p)].append(
                ('xtsgnd', (t, s, g, n, d)))
            self.graph['xtsgndp'][(t, s, g, n, d, p)].append(
                ('xtdp', (t, d, p)))
            self.graph['xtsgndp'][(t, s, g, n, d, p)].append(
        ...
        for (t, s, g, n, d) in list(self.graph['xtsgnd'].keys()):
            or_list = []
            for p in StaticVariables.periods[d]:
                or_list.append(('xtsgndp', (t, s, g, n, d, p)))
            self.graph['xtsgnd'][(t, s, g, n, d)].append(
                (StaticVariables.or_head, or_list))
        ...
        for (t, d, p) in list(self.graph['xtdp'].keys()):
            or_list_t[(t, d, p)] = []

        for (g, d, p) in list(self.graph['xgdp'].keys()):
            or_list_g[(g, d, p)] = []
        ...
        for (t, d, p) in list(self.graph['xtdp'].keys()):
            ...
            self.graph['xtdp'][(t, d, p)].append(
                (StaticVariables.or_head, or_list_t[(t, d, p)]))

        for (g, d, p) in list(self.graph['xgdp'].keys()):
            self.graph['xgdp'][(g, d, p)].append(
                (StaticVariables.or_head, or_list_g[(g, d, p)]))

        for (t, d) in list(self.graph['xtd'].keys()):
            or_list = []
            for p in StaticVariables.periods[d]:
                or_list.append(('xtdp', (t, d, p)))
            self.graph['xtd'][(t, d)].append(
                (StaticVariables.or_head, or_list))

        for (t, p) in list(self.graph['xtp'].keys()):
            or_list = []
            for d in StaticVariables.days:
                if bic3(d, p):
                    or_list.append(('xtdp', (t, d, p)))
            self.graph['xtp'][(t, p)].append(
                (StaticVariables.or_head, or_list))

        for (k, t, d, p) in list(self.graph['iktdp'].keys()):
            self.graph['iktdp'][(k, t, d, p)].append(('xtdp', (t, d, p - 1)))
            or_list = []
            for j in range(k):
                or_list.append(('xtdp', (t, d, p + j)))
                self.graph['iktdp'][(k, t, d, p)].append(
                    negation(('xtdp', (t, d, p + j))))
        ...
        for (k, t, d) in list(self.graph['iktd'].keys()):
            or_list = []
            for p in StaticVariables.periods[d]:
                if bic10(k, d, p):
                    or_list.append(('iktdp', (k, t, d, p)))
            self.graph['iktd'][(k, t, d)].append(
                (StaticVariables.or_head, or_list))
        ...
        for (k, t) in list(self.graph['ikt'].keys()):
            or_list = []
            for d in StaticVariables.days:
                or_list.append(('iktd', (k, t, d)))
            self.graph['ikt'][(k, t)].append(
                (StaticVariables.or_head, or_list))

        for (t, d, p) in list(self.graph['itdp'].keys()):
            or_list = []
            for k in range(StaticVariables.p_max):
                if bic12(k, d, p):
                    or_list.append(('iktdp', (k, t, d, p)))
            self.graph['itdp'][(t, d, p)].append(
                (StaticVariables.or_head, or_list))

        # ================ EXACTLY SAME FOR GROUPS =================
        # Abstract this somehow!

        for (k, g, d, p) in list(self.graph['ikgdp'].keys()):
            self.graph['ikgdp'][(k, g, d, p)].append(('xgdp', (g, d, p - 1)))
            or_list = []
            for j in range(k):
                or_list.append(('xgdp', (g, d, p + j)))
                self.graph['ikgdp'][(k, g, d, p)].append(
                    negation(('xgdp', (g, d, p + j))))
        ...
        for (k, g, d) in list(self.graph['ikgd'].keys()):
            or_list = []
            for p in StaticVariables.periods[d]:
                if bic10(k, d, p):
                    or_list.append(('ikgdp', (k, g, d, p)))
            self.graph['ikgd'][(k, g, d)].append(
                (StaticVariables.or_head, or_list))
        ...
        for (k, g) in list(self.graph['ikg'].keys()):
            or_list = []
            for d in StaticVariables.days:
                or_list.append(('ikgd', (k, g, d)))
            self.graph['ikg'][(k, g)].append(
                (StaticVariables.or_head, or_list))

        for (g, d, p) in list(self.graph['igdp'].keys()):
            or_list = []
            for k in range(StaticVariables.p_max):
                if bic12(k, d, p):
                    or_list.append(('ikgdp', (k, g, d, p)))
            self.graph['igdp'][(g, d, p)].append(
                (StaticVariables.or_head, or_list))
    ...
    def correctness_implications(self):
        ...
        for (t, s, g, n) in list(multi_dict.keys()):
            self.true_list.append(single(multi_dict[(t, s, g, n)]))

        multi_dict = {}
        for (t, s, g, n, d, p) in list(self.graph['x!tsgndp'].keys()):
            multi_dict[(t, s, g, n, d)] = []
        for (t, s, g, n, d, p) in list(self.graph['x!tsgndp'].keys()):
            multi_dict[(t, s, g, n, d)].append(
                ('x!tsgndp', (t, s, g, n, d, p)))
        for (t, s, g, n, d) in list(multi_dict.keys()):
        ...
        for (g, d, p) in list(multi_dict.keys()):
        ...
        for (t, s, g, n, d, p) in list(self.graph['xtsgndp'].keys()):
            multi_dict[(t, d, p)][(s, n)].append(
                ('xtsgndp', (t, s, g, n, d, p)))
        for (t, d, p) in list(multi_dict.keys()):
            single_list = []
            for (s, n) in multi_dict[(t, d, p)]:
                ...
                for i in range(l):
                    if i == 0:
                        ...
                        append_list = multi_dict[(t, d, p)][(
                            s, n)][:i - 1] + multi_dict[(t, d, p)][(s, n)][i + 1:]
                    self.graph['xtsgndp'][multi_dict[(
                        t, d, p)][(s, n)][i][1]] += append_list
