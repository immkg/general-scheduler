    ...
    if not favoured_hours:  # dict = {(t,s,g,n):[p]}
        ...
        for (t, s, g, n) in list(favoured_hours.keys()):
            for d in days:
                ...
                comfort_graph['xtsgnd'][(t, s, g, n, d)].append(
                    ('or', or_list))

    # first or last last lesson in a day: 'x!tsgndp' => (and,
    # negation('xgdp')) or ('and', negation('xgdp'))
        ...
        for (t, s, g, n) in last_first_hours:
            for d in days:
                for p in periods[d]:
                    ...
                    and_list2 = []
                    for p2 in range(
                            p + duration[(t, s, g, n)], max(periods(d) + 1)):
                        and_list2.append(negation(('xgdp', (g, d, p))))

                    or_list = [('and', and_list1), ('and', and_list2)]

                    comfort_graph['x!tsgndp'][(t, s, g, n, d, p)].append(
                        ('or', or_list))
